...
<div class='${HtmlReportUtils.getBehaviorDrivenTypeName(node)?lower_case} node' test-id='${node.getId()}' status='${node.status}'>
	<#if node.hasCategory()>
	...
	<div class="bdd-test">
		<div class="scenario-name"><span class='status ${node.status?lower_case}' title='${node.status}'><i class='material-icons'>${MaterialIcon.getIcon(node.status)}</i></span> ${HtmlReportUtils.getBehaviorDrivenTypeName(node)}: ${node.name}</div>
	...
	<ul class='steps'>
		...
		<li test-id='${child.getId()}' class='node ${HtmlReportUtils.getBehaviorDrivenTypeName(child)?lower_case} ${child.status}' status='${child.status}'>
			...
			<ul class='gc steps'>
				...
				<li test-id='${gc.getId()}' class='gc ${HtmlReportUtils.getBehaviorDrivenTypeName(gc)?lower_case} ${gc.status}' status='${gc.status}'>
